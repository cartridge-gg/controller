...
const register =
  () =>
    async (
      ...
      const address = computeAddress(username, { x0, x1, x2 }, { y0, y1, y2 }, deviceKey);
...
const computeAddress = (username: string, { x0, x1, x2 }: { x0: BN, x1: BN, x2: BN }, { y0, y1, y2 }: { y0: BN, y1: BN, y2: BN }, deviceKey: string) => hash.calculateContractAddressFromHash(
