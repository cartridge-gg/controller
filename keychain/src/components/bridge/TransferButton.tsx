...
import { goerli, mainnet, useContractWrite, usePrepareContractWrite } from "wagmi";
import { EthL1BridgeGoerli, EthL1BridgeMainnet, EthL2BridgeGoerli, EthL2BridgeMainnet } from "./constants";
...
const TransferButton = ({
  ...
  const estimateL2Fee = useCallback(async (): Promise<Sequencer.EstimateFeeResponse> => {
    const gateway = new SequencerProvider({
      network: account._chainId === constants.StarknetChainId.MAINNET
        ? "mainnet-alpha"
        : "goerli-alpha",
    });
    const parsed = parseEther(value);
    const amount = uint256.bnToUint256(new BN(parsed.toString()));
    const from = account._chainId === constants.StarknetChainId.MAINNET
      ? EthL1BridgeMainnet
      : EthL1BridgeGoerli
    const to = account._chainId === constants.StarknetChainId.MAINNET
      ? EthL2BridgeMainnet
      : EthL2BridgeGoerli
  ...
  const { config, error: configError } = usePrepareContractWrite({
    chainId: account._chainId === constants.StarknetChainId.MAINNET
      ? mainnet.id
      : goerli.id,
    address: account._chainId === constants.StarknetChainId.MAINNET
      ? EthL1BridgeMainnet
      : EthL1BridgeGoerli,
    abi: EthL1BridgeABI,
