...
import { getTransactionReceipt } from "./rpc";
  ...
  async sync() {
    ...
    try {
      if (
        ...
        // cartridge rpc returns "txn hash not found" of txn that has failed/rejected,
        // uses starknet's feeder gateway for now
        const deployTxnReceipt = await getTransactionReceipt(
          this._chainId,
          deployTxnHash,
        );
