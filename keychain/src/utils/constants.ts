export const PROXY_CLASS =
  "0x04be79b3904b4e2775fd706fa037610b41d8f8708ce298aac3a470badf68176d";
export const ACCOUNT_CLASS =
  "0x079507b6068846a05500331e0b54f7b539f067c2003de1f4635fc9885a267144";
export const CONTROLLER_CLASS =
  "0x0286a2ea79ee08506efcbc330efd2ae34e2f22b79ecd2fb9b86ce26d6a1dbece";
export const LEGACY_CONTROLLER_CLASS =
  "0x58e648a242085d5ff1e8f92a6b91057826639a82b23798998d40b61a27bca85";

export const RPC_MAINNET = "https://starknet.cartridge.gg/rpc/v0.1";
export const RPC_GOERL = "https://starknet-goerli.cartridge.gg/rpc/v0.1";
export const RPC_GOERL2 = "https://starknet-goerli2.cartridge.gg/rpc/v0.1";

export const GATEWAY_MAINNET = process.env.NEXT_PUBLIC_GATEWAY_MAINNET;
export const GATEWAY_GOERLI = process.env.NEXT_PUBLIC_GATEWAY_GOERLI;

export const ETH_RPC_MAINNET = process.env.NEXT_PUBLIC_ETH_RPC_MAINNET;
export const ETH_RPC_GOERLI = process.env.NEXT_PUBLIC_ETH_RPC_GOERLI;
