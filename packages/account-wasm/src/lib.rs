...
use deployment::JsDeploymentBuilder;
...
impl CartridgeAccount {
    ...
    pub fn external_deployment_calls(
        ...
        let owner = from_value(owner)?;
        let username = from_value(username)?;
        let methods = policies
            .into_iter()
            .map(TryFrom::try_from)
            .collect::<std::result::Result<Vec<_>, _>>()?;

        JsDeploymentBuilder::new(owner, username)
            .with_allowed_methods(methods, expires_at)
            .with_delegate_account(owner)
            .with_initial_deposit(initial_deposit)
            .build()
    }
}
