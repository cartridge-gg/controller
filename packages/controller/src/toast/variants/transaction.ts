...
export function injectTransactionStyles(targetDoc: Document): void {
  ...
  style.textContent = `
    /* Transaction Toast */
    .cartridge-toast.transaction true {
      background-color: #161A17;
      border-radius: 8px;
      width: 360px;
    }

    .cartridge-toast.transaction .label-bar {
      display: flex;
      padding: 12px;
      align-items: center;
      gap: 8px;
    ...
    .cartridge-toast.transaction .label-bar .label-container {
      display: flex;
      align-items: center;
      gap: 8px;
    }

    .cartridge-toast.transaction .label-bar .label-container .icon-container {
      width: 24px;
      height: 24px;
      aspect-ratio: 1/1;
    }

    .cartridge-toast.transaction .label-bar .label-container p.status {
      color: #FFF;
      /* Inter/Medium 14px */
      font-family: Inter;
      font-size: 14px;
    ...
    .cartridge-toast.transaction .label-bar .label-container .activity-feed-container {
      display: flex;
      padding: 2px;
      align-items: center;
      border-radius: 2px;
    ...
    .cartridge-toast.transaction .label-bar .label-container .activity-feed-container .activity-icon {
      width: 16px;
      height: 16px;
      aspect-ratio: 1/1;
    }

    .cartridge-toast.transaction .label-bar .label-container .activity-feed-container .activity-label-container {
      display: flex;
      padding: 0 2px;
    ...
    .cartridge-toast.transaction .label-bar .label-container .activity-feed-container span.activity-label {
      color: var(--bg-theme-100, #3F3);
      /* Inter/Regular 12px */
      font-family: Inter;
      font-size: 12px;
      font-style: normal;
    ...
    .cartridge-toast.transaction false {
      display: inline-flex;
      padding: 10px;
    ...
    .cartridge-toast.transaction true .close-button-container {
      display: flex;
      padding: 4px;
      align-items: center;
      gap: 10px;
    ...
    .cartridge-toast.transaction false .collapsed-icon {
      width: 28px;
      height: 28px;
      aspect-ratio: 1/1;
    }
...
export function createTransactionToast(
  ...
  toast.className = `cartridge-toast transaction ${options.isExpanded}`;
  ...
  if (options.isExpanded) {
    ...
    if (options.status === "confirming") {
      ...
      iconContainer.innerHTML = `
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
          <path d="M8.36382 18.5465L4 14.1827L5.45427 12.7284L8.36382 15.638L18.5457 5.45508L20 6.91032L8.36382 18.5465Z" fill="white"/>
        </svg>
      `;
    }
