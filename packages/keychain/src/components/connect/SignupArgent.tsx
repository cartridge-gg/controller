...
function SignupArgentInner() {
  ...
  const { tokens, isAllFunded, isChecked, isFetching } = useTokens({
    extAccount: extAccount as Account,
  });
...
function useTokens({ extAccount }: { extAccount: Account }) {
  ...
  const checkFunds = useCallback(async () => {
    ...
    const checked = await updateBalance(tokens, extAccount);
    setTokens(checked);
