// TODO: #223 Don't forget to remove this module when upgrading to starknet.js v5
//
import { BN } from "bn.js";
import { BlockNumber, SequencerIdentifier, number } from "starknet";
 ...
 * @param hashField
 */
export function formatHash(hashValue: number.BigNumberish): string {
  if (typeof hashValue === "string") return hashValue;
  return number.toHex(number.toBN(hashValue));
}
...
export type BlockIdentifier = BlockNumber | number.BigNumberish;
...
export class Block {
  ...
  private setIdentifier(__identifier: BlockIdentifier) {
    if (typeof __identifier === "string" && number.isHex(__identifier)) {
      this.hash = __identifier;
    } else if (BN.isBN(__identifier)) {
      this.hash = number.toHex(__identifier as BN);
    } else if (typeof __identifier === "number") {
      this.number = __identifier;
    } else if (
      typeof __identifier === "string" &&
      validBlockTags.includes(__identifier)
    ) {
