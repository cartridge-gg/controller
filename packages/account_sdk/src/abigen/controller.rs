...
impl cainome::cairo_serde::CairoSerde for Event {
    ...
    fn cairo_deserialize(
        ...
        match __index as usize {
            ...
            3usize => Ok(Event::TransactionExecuted(
                TransactionExecuted::cairo_deserialize(__felts, __offset + 1)?,
            )),
            4usize => Ok(Event::MultipleOwnersEvent(
                MultipleOwnersEvent::cairo_deserialize(__felts, __offset + 1)?,
            )),
            5usize => Ok(Event::ReentrancyGuardEvent(
                ReentrancyGuardEvent::cairo_deserialize(__felts, __offset + 1)?,
            )),
            6usize => Ok(Event::SessionEvent(SessionEvent::cairo_deserialize(
                __felts,
                __offset + 1,
            )?)),
            7usize => Ok(Event::ExternalOwnersEvent(
                ExternalOwnersEvent::cairo_deserialize(__felts, __offset + 1)?,
            )),
            8usize => Ok(Event::ExecuteFromOutsideEvents(
                OutsideExecutionEvent::cairo_deserialize(__felts, __offset + 1)?,
            )),
            9usize => Ok(Event::DelegateAccountEvents(
                DelegateAccountEvent::cairo_deserialize(__felts, __offset + 1)?,
            )),
            10usize => Ok(Event::SRC5Events(Src5ComponentEvent::cairo_deserialize(
                __felts,
                __offset + 1,
            )?)),
            11usize => Ok(Event::UpgradeableEvent(UpgradeEvent::cairo_deserialize(
                __felts,
                __offset + 1,
            )?)),
            _ => {
