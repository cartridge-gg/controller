...
use crate::{
    ...
    account::session::{
        hash::{CallPolicy, Policy, Session},
        SessionAccount,
    },
    artifacts::Version,
...
pub async fn test_verify_execute(owner: Owner) {
    ...
    let policies = vec![
        Policy::Call(CallPolicy::new(*FEE_TOKEN_ADDRESS, selector!("tdfs"))),
        Policy::Call(CallPolicy::new(*FEE_TOKEN_ADDRESS, selector!("transfds"))),
        Policy::Call(CallPolicy::new(*FEE_TOKEN_ADDRESS, selector!("transfer"))),
    ];
...
async fn test_create_and_use_registered_session() {
    ...
    let policies = vec![
        Policy::Call(CallPolicy::new(*FEE_TOKEN_ADDRESS, selector!("transfer"))),
        Policy::Call(CallPolicy::new(*FEE_TOKEN_ADDRESS, selector!("approve"))),
    ];
