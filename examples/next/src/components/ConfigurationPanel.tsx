...
export function ConfigurationPanel() {
  ...
  const handleApplyConfiguration = async () => {
    ...
    try {
      // If connected, disconnect first so user can reconnect with new config
      if (address) {
        await disconnect();
      }

      // Update the configuration context
      // This will create new connector instances with updated options
      updateConfig({
        preset,
        slot,
        namespace,
        ...
        {hasChanges && (
          <div className="alert alert-info">
            ...
            <span className="text-sm">
              Configuration changes apply to new connections. Connect your
              wallet after applying to use the new settings.
            </span>
          </div>
        )}
      </div>
