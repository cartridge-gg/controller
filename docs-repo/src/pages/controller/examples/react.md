...
**New in v1.0+**: The connector now works with minimal configuration thanks to automatic default chain inclusion.
...
For simple applications, you can use the connector with zero configuration:

```typescript
import ControllerConnector from "@cartridge/connector/controller";

// Minimal setup - uses default chains and mainnet
...
// Define session policies (optional)
...
// Initialize the connector with simplified configuration
// Default chains (mainnet, sepolia) are automatically included
const connector = new ControllerConnector({
  policies, // Optional - can be omitted for minimal setup
  // RPC configuration is optional - defaults are provided
})

// Configure RPC provider (must use Cartridge RPC for mainnet/sepolia)
